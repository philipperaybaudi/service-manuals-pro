// Import 3M documents
// Usage: node scripts/import-3m.mjs
...
const SOURCE_DIR = 'C:/Users/adm/Documents/SHEMATHEQUE/DOSSIER SOURCE/Photography/3M';
...
const PHOTOGRAPHY_CAT_SLUG = 'photography';

const docs = [
  {
    file: '3m_projector_2000.pdf',
    title: '3M 2000 Overhead Projector Service Manual',
    slug: '3m-2000-overhead-projector-service-manual',
    overlayTitle: '3M 2000 Overhead Projector — Service Manual',
    price: 1500,
    description: 'Service manual for the 3M 2000 Overhead Projector (Visual Systems Division). Covers introduction and typical specifications, operation and maintenance procedures (setup, fold-up procedure, cleaning, lamp replacement), troubleshooting guide, and adjustment procedures including projector checkout.',
  },
  {
    file: '3m_projector_9000.pdf',
    title: '3M 9000 Series Overhead Projector Service Manual',
    slug: '3m-9000-series-overhead-projector-service-manual',
    overlayTitle: '3M 9000 Series Overhead Projector — Service Manual',
    price: 1500,
    description: 'Service manual for the 3M 9000 Series Overhead Projectors, covering models 9050, 9060, 9070, 9075, 9080, 9100, 9200, and 9550. Covers overview, parts identification, physical dimensions, specifications, principles of operation (optical components, controls, lamp changer and high/low controls, projector-to-screen distance calculation, color tube), operating procedures, and servicing.',
  },
];

async function run() {
  console.log('=== Import 3M ===\n');
  if (!existsSync(TEMP_DIR)) mkdirSync(TEMP_DIR, { recursive: true });

  // Get photography category ID
  const { data: cat } = await supabase.from('categories').select('id').eq('slug', PHOTOGRAPHY_CAT_SLUG).single();
  if (!cat) { console.error('Category "photography" not found'); process.exit(1); }
  const catId = cat.id;

  // Create brand
  console.log('1. Création brand 3M...');
  const { data: brand, error: brandErr } = await supabase.from('brands').insert({
    name: '3M', slug: '3m', category_id: catId,
  }).select().single();

  let brandId;
  if (brandErr?.code === '23505') {
    const { data: existing } = await supabase.from('brands').select('id').eq('slug', '3m').single();
    brandId = existing.id;
  ...
  for (const doc of docs) {
    ...
    try {
      ...
      const { data: dbDoc, error: docErr } = await supabase.from('documents').insert({
        ...
        seo_description: `Download the ${doc.title}. Technical documentation with specifications, troubleshooting and repair procedures.`,
        language: 'en',
        active: true,
        featured: false,
      }).select().single();
