// Import Bernardo documents
// Usage: node scripts/import-bernardo.mjs
...
const SOURCE_DIR = 'C:/Users/adm/Documents/SHEMATHEQUE/DOSSIER SOURCE/Machines/BERNARDO';
...
const docs = [
  {
    file: 'Bernardo PT260 degau - rabot.pdf',
    title: 'Bernardo PT260 User Manual',
    slug: 'bernardo-pt260-user-manual',
    overlayTitle: 'Bernardo PT260 — User Manual',
    price: 1500,
    description: 'User manual for the Bernardo PT260 combination jointer-planer (dégauchisseuse-raboteuse). Covers technical specifications (254 mm jointing width, 2-knife cutter block), safety rules, unpacking, machine composition, installation, electrical diagram, parallel fence setup, chip extractor connection, jointing and planing procedures, blade replacement, maintenance, troubleshooting, and illustrated assembly diagram with parts list. Document in French.',
  },
];

async function run() {
  console.log('=== Import Bernardo ===\n');
  if (!existsSync(TEMP_DIR)) mkdirSync(TEMP_DIR, { recursive: true });

  // Create brand
  console.log('1. Création brand BERNARDO...');
  const { data: brand, error: brandErr } = await supabase.from('brands').insert({
    name: 'Bernardo', slug: 'bernardo', category_id: MACHINE_TOOLS_CAT_ID,
  }).select().single();

  let brandId;
  if (brandErr?.code === '23505') {
    const { data: existing } = await supabase.from('brands').select('id').eq('slug', 'bernardo').single();
    brandId = existing.id;
  ...
  for (const doc of docs) {
    ...
    try {
      ...
      const { data: dbDoc, error: docErr } = await supabase.from('documents').insert({
        ...
        seo_description: `Download the ${doc.title}. User manual with specifications, installation, operating and maintenance instructions.`,
        language: 'fr',
        active: true,
        featured: false,
      }).select().single();
