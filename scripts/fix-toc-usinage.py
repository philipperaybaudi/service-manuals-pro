"""
fix-toc-usinage.py
Ajoute le TOC (via Vision Claude Haiku) aux 7 docs Usinage >= $20.
PDFs locaux dans DOCS EN LIGNE/Usinage/USINAGE/

Usage :
  python -X utf8 scripts/fix-toc-usinage.py --dry-run
  python -X utf8 scripts/fix-toc-usinage.py
"""
...
PDF_DIR = Path(r'C:\Users\adm\Documents\SHEMATHEQUE\DOCS EN LIGNE\Usinage\USINAGE')

# Slug → nom de fichier exact
DOCS = {
    'usinage-carborundum-principles-of-tool-room-grinding-manual':
        'Carborundum-Principles-Tool-Room-Grinding-Manual.pdf',
    'usinage-usda-small-sawmill-operators-manual-agriculture-handbook-27':
        'USDA-Small-Sawmill-Operators-Manual-AgHandbook27-1952.pdf',
    'usinage-cincinnati-milling-machine-co-treatise-on-milling-and-milling-machines':
        'Cincinnati-Milling-Machine-Co-Treatise-on-Milling.pdf',
    'usinage-brown-sharpe-treatise-construction-use-milling-machines-1883':
        'Brown-Sharpe-Treatise-Construction-Use-Milling-Machines-1883.pdf',
    'usinage-vereina-krasnov-metal-cutting-machines-textbook':
        'Vereina-Krasnov-Metallorejushchie-Stanki-Textbook-RU.pdf',
    'usinage-lathe-design-construction-and-operation-technical-book':
        'Lathe-Design-Construction-and-Operation-Technical-Book.pdf',
    'usinage-machine-shop-tools-methods-reference-book':
        'Machine-Shop-Tools-Methods-Reference-Book.pdf',
}

VISION_PROMPT = (
    "Look at this page carefully.\n\n"
    "If this is a Table of Contents (or index / sommaire / Inhaltsverzeichnis / оглавление), "
...
def vision_extract_toc(pdf_path: Path, max_pages: int = 20) -> tuple:
...
print(f'\n{"="*65}')
print(f'  TOC USINAGE >= $20 — {len(DOCS)} docs{"  [DRY-RUN]" if DRY_RUN else ""}')
print(f'{"="*65}\n')
...
for slug, filename in DOCS.items():
    ...
    if 'Table of Contents:' in desc_en:
        print(f'  ○ TOC déjà présent — ignoré')
        stats['ok'] += 1; continue

    # Vision
    print(f'  Vision scan (max 20 pages)...')
    try:
        lines_en, lines_fr = vision_extract_toc(pdf_path, max_pages=20)
    except Exception as e:
    ...
    for l in lines_en[:4]: print(f'    EN • {l}')
    if len(lines_en) > 4: print(f'    EN   ... ({len(lines_en)-4} de plus)')
