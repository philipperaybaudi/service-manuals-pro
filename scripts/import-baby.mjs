// Import Baby documents
// Usage: node scripts/import-baby.mjs
...
const SOURCE_DIR = 'C:/Users/adm/Documents/SHEMATHEQUE/DOSSIER SOURCE/Machines/BABY';
...
const docs = [
  {
    file: 'Baby 1130 $10.pdf',
    title: 'Baby 1130 Assembly & User Guide',
    slug: 'baby-1130-assembly-user-guide',
    overlayTitle: 'Baby 1130 — Assembly & User Guide',
    price: 1000, // $10
    description: 'Assembly and user guide for the Baby 1130 combination woodworking machine (machines à bois). Covers motor and pulley alignment, belt routing for each operation, assembly of the circular saw, router, mortiser and adjustable fence, and detailed operating instructions for all functions: jointer (dégauchisseuse), thickness planer (raboteuse), circular saw, router (toupie) and mortiser. Document in French.',
  },
];

async function run() {
  console.log('=== Import Baby ===\n');
  if (!existsSync(TEMP_DIR)) mkdirSync(TEMP_DIR, { recursive: true });

  // 1. Create brand
  console.log('1. Création brand BABY...');
  const { data: brand, error: brandErr } = await supabase.from('brands').insert({
    name: 'Baby', slug: 'baby', category_id: MACHINE_TOOLS_CAT_ID,
  }).select().single();

  let brandId;
  if (brandErr?.code === '23505') {
    const { data: existing } = await supabase.from('brands').select('id').eq('slug', 'baby').single();
    brandId = existing.id;
  ...
  for (const doc of docs) {
    ...
    try {
      ...
      const { data: dbDoc, error: docErr } = await supabase.from('documents').insert({
        ...
        seo_description: `Download the ${doc.title}. Technical documentation for the Baby 1130 woodworking machine.`,
        language: 'en',
        active: true,
        featured: false,
      }).select().single();
