/**
 * Extraction TdM pour : the_manufacture_of_opium $25.pdf
 * slug : ouvrages-de-reference-the-truth-about-opium-smoking
 */
...
const PDF_PATH = 'C:\\Users\\adm\\Documents\\SHEMATHEQUE\\DOCS EN LIGNE\\Nature\\OUVRAGES DE RÉFÉRENCE\\the_manufacture_of_opium $25.pdf';
  ...
  const response = await anthropic.messages.create({
    ...
    messages: [{
      ...
      content: [
        ...
        { type: 'text', text: `This is "${title}" (1882 edition).
Find the table of contents page(s) in this PDF and extract ALL entries.

Return ONLY a JSON object with exactly two keys:
- "toc_en": the table of contents in English (original or translated), plain text list (one entry per line, format: "- Chapter/section title .... page N")
...
- Return ONLY valid JSON, no other text` }
  ...
  if (!jsonStr) { console.log('  RÉPONSE COMPLÈTE :\n' + raw); throw new Error('Réponse non parseable'); }
  return JSON.parse(jsonStr);
}

// ── Main ──
if (!fs.existsSync(PDF_PATH)) { console.error(`Fichier introuvable : ${PDF_PATH}`); process.exit(1); }
...
let toc;
try { toc = await extractTOC(pdfBuffer, row.title); }
catch (e) { console.error(`\nErreur : ${e.message}`); process.exit(1); }

if (!toc.toc_en) { console.log('⚠ Pas de TdM trouvée'); process.exit(0); }
