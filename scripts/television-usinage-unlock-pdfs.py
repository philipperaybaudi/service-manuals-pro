"""
television-usinage-unlock-pdfs.py — Phase 0 : déverrouillage PDFs Télévision + Usinage
...
SOURCE_DIRS = [
    Path(r'C:\Users\adm\Documents\SHEMATHEQUE\DOSSIER SOURCE\Catégories\Télévision'),
    Path(r'C:\Users\adm\Documents\SHEMATHEQUE\DOSSIER SOURCE\Catégories\Usinage'),
]
...
for SOURCE_DIR in SOURCE_DIRS:
    ...
    print(f'\n{"="*60}')
    print(f'  {SOURCE_DIR.name.upper()} — PHASE 0 DÉVERROUILLAGE ({len(all_pdfs)} PDFs)')
    print(f'{"="*60}\n')
    ...
    print(f'\n  BILAN : {unlocked} déverrouillés | {already_ok} déjà OK | {len(errors)} erreurs')
    if errors:
        print(f'  ERREURS :')
        for e in errors:
            print(f'    {e}')
