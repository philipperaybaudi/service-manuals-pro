/**
 * Correction des 3 docs Post-Lot-1 avec langue mélangée dans description_fr.
 * Le script lit chaque PDF depuis R2, génère une description FR propre via Claude,
 * et met à jour uniquement description + description_fr (title/title_fr intouchables).
 *
 * Usage : node scripts/fix-post-lot1-langue-melangee.mjs [--dry-run]
 */
...
const SLUGS = [
  'bang-olufsen-bang-olufsen-beocenter-3300-type-1604-service-manual',
  'de-dietrich-de-dietrich-dop750-user-guide',
  'de-dietrich-de-dietrich-dop870-user-guide',
];
...
1. **description_fr** : Description en français UNIQUEMENT (150-250 mots) basée sur ce que tu vois dans le PDF. Mentionne les systèmes, chapitres ou sections visibles.

2. **description_en** : La même description en anglais, rédaction originale (pas traduction mot-à-mot).

RÈGLES ABSOLUES :
- La description_fr doit être ENTIÈREMENT en français — aucun mot anglais
...
async function processDoc(doc) {
  console.log(`\n→ ${doc.slug}`);
  console.log(`  Titre EN : ${doc.title}`);
  console.log(`  title_fr : ${doc.title_fr || '(absent)'}`);
...
// ─── Main ────────────────────────────────────────────────────────────────────
console.log(`\n=== Fix Post-Lot-1 langue mélangée (${DRY_RUN ? 'DRY-RUN' : 'PRODUCTION'}) — ${SLUGS.length} docs ===\n`);
