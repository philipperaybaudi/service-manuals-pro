// Import Aaton documents
// Usage: node scripts/import-aaton.mjs
...
const SOURCE_DIR = 'C:/Users/adm/Documents/SHEMATHEQUE/DOSSIER SOURCE/Photography/AATON';
...
const docs = [
  {
    file: 'aaton 35mn Notice GB.pdf',
    title: "Aaton 35-III User's Guide",
    slug: 'aaton-35-iii-users-guide',
    overlayTitle: "Aaton 35-III — User's Guide",
    price: 1500,
    description: "User's guide for the Aaton 35-III professional 35mm film camera. Covers camera introduction and overview, operating procedures, controls and adjustments, loading and film handling, maintenance, and technical specifications. Published June 1997 by Aaton, Grenoble, France.",
  },
];

async function run() {
  console.log('=== Import Aaton ===\n');
  ...
  console.log('1. Création brand AATON...');
  const { data: brand, error: brandErr } = await supabase.from('brands').insert({
    name: 'Aaton', slug: 'aaton', category_id: catId,
  }).select().single();

  let brandId;
  if (brandErr?.code === '23505') {
    const { data: existing } = await supabase.from('brands').select('id').eq('slug', 'aaton').single();
    brandId = existing.id;
  ...
  for (const doc of docs) {
    ...
    try {
      ...
      const { data: dbDoc, error: docErr } = await supabase.from('documents').insert({
        ...
        seo_description: `Download the ${doc.title}. Complete user's guide for the Aaton 35-III professional 35mm film camera.`,
        language: 'en',
        active: true,
        featured: false,
      }).select().single();
